...
public class Http2ServerInitializer extends ChannelInitializer<SocketChannel> {
    ...
    public void initChannel(SocketChannel ch) {
        ...
        p.addLast(new HttpServerUpgradeHandler(sourceCodec, upgradeCodecFactory)); // 如果是升级的请求
        p.addLast(new Http1InboundHandler(maxHttpContentLength)); // 如果是原始HTTP/1.1的请求

        p.addLast(new UserEventLogger());
    }

    private static class Http1InboundHandler extends SimpleChannelInboundHandler<HttpMessage> {
        private int maxHttpContentLength;
        public Http1InboundHandler(int maxHttpContentLength){
            this.maxHttpContentLength = maxHttpContentLength;
        }
        @Override
